...
<head>
  ...
  <style>
    ...
    * {
    ...
    .hero-front-svg-stage.is-loading {
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .hero-front-svg-stage.is-loading::after {
      content: '';
      width: 40px;
      height: 40px;
      border-radius: 50%;
  ...
  <script>
    ...
    function showHeroFrontView(viewKey) {
      ...
      if (view.interactive && heroFrontSvgStage) {
        // Abrir la vista inmediatamente con spinner para feedback visual instantáneo
        heroFrontImage.style.display = 'none';
        heroFrontSvgStage.innerHTML = '';
        heroFrontSvgStage.classList.add('active', 'is-loading');
        heroFrontSvgStage.classList.remove('is-step-2', 'is-two-rooms-layout', 'step2-animate');
        heroFrontSvgStage.setAttribute('aria-hidden', 'false');
        heroFrontView.classList.remove('step-2-focus');
        heroFrontView.classList.add('layout-selection-focus');
        if (heroFrontTitle) heroFrontTitle.textContent = 'Loading...';
        if (heroFrontClose) heroFrontClose.textContent = 'Return to site map';
        revealHeroFrontView();

        fetchSvgCached(view.image)
          .then((svgText) => {
            heroFrontSvgStage.classList.remove('is-loading');
          ...
          .catch(() => {
            heroFrontSvgStage.classList.remove('active', 'is-loading');
            heroFrontSvgStage.classList.remove('is-two-rooms-layout');
            heroFrontSvgStage.setAttribute('aria-hidden', 'true');
            heroFrontView.classList.remove('layout-selection-focus', 'step-2-focus');
